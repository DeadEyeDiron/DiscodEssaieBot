...
module.exports = class SetModeratorCommand extends Command {
    ...
    async run(msg) {
        const hasRoleModerator = msg.guild.roles.cache.some(role => role.name === 'modo'); // Adaptez le nom si vous changez le nom du rôle modérateur
        if (!hasRoleModerator) {
            this.client.logger.log('error', `Role 'modo' non trouvé sur ce serveur.`);
            return;
        }

        const roleModerator = msg.guild.roles.cache
            .filter(role => role.name === 'modo')
            .first();

        msg.guild.channels.cache.map(channel => {
            this.client.logger.log('info', `Role 'modo' ajouté sur ${channel.name}.`);

            // On surcharge les permissions du rôle 'modo' sur chaque channel du serveur :
